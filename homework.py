...
class Training:
    ...
    def get_distance(self) -> float:
        ...
        dist_1:float=self.action* self.LEN_STEP/ self.M_IN_KM 
        return dist_1  # vrode kak dolzhen vernyt 
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий.""" # tyt nichego ne nado pass
        pass
...
class Running(Training):
    ...
    def  get_spent_calories(self) -> float:  
        ...
        calorie_1:float =(self.coeff_1* self.get_mean_speed()-self.coeff_2)*self.weight / self.M_IN_KM *(self.duration*self.M_IN_HOUR)  
        return calorie_1
...
class SportsWalking(Training):
    ...
    def  get_spent_calories(self) -> float:
        ...
        calorie_2:float=(self.coeff_3*self.weight + (self.get_mean_speed()**self.coeff_5// self.height)*self.coeff_4*self.weight)*(self.duration*self.M_IN_HOUR)
        return calorie_2  
...
class Swimming(Training):
    ...
    def get_mean_speed(self)->float:
        m_speed :float= self.length_pool*self.count_pool/ self.M_IN_KM /self.duration
        return m_speed #poschital i vernul
    def  get_spent_calories(self) -> float:
        
        calorie_3:float= (self.get_mean_speed()+self.coeff_6)*self.coeff_7*self.weight
